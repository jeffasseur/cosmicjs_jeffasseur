        ...
        <div key={blog.id} className="col-12 md:col-6 lg:col-4 item group">
          ...
          <div className="card blog-item">
            <div className="image-holder aspect-square overflow-hidden">
              {/* Card Thumbnail */}
              <a
                className="card-thumb object-cover rounded-md overflow-hidden"
                href={`/blog/${blog.slug}`}
              >
                <img
                  src={blog.metadata.image.imgix_url}
                  alt={blog.title}
                  className="h-full w-full object-cover rounded-md group-hover:scale-110 transition-all duration-500 ease-in-out"
                />
              </a>
              <div className="card-overlay top fade-down">
                <div className="logo">
                  <img
                    src={blog.metadata.author.metadata.image.imgix_url}
                    alt="Logo"
                  />
                </div>
                <div className="post-meta d-flex flex-column ms-3">
                  ...
                  <span className="post-author">
                    <strong>{blog.metadata.author.title}</strong>
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="heading flex flex-col gap-1">
                ...
                <div className="card-terms">
                  {blog.metadata.categories.map((category, index) => (
                    <p
                      key={index}
                      className="bg-light-80 px-4 py-2 rounded-full hover:bg-light-90 hover:translate-y-1 transition-all duration-300 ease-in-out"
                    >
                      {category.title}
                    </p>
                  ))}
